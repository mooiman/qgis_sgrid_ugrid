...
\memoTelephone{---}
\memoEmail{---}
\memoSubject{Manual to plot SGRID and/or UGRID map-files with QGIS}
\memoCopy{}
%
\mooimantitle
%\phantom{text}
...
%\begin{tabular}{@{}p{40.0mm}@{}p{0mm}p{\textwidth-27mm-24pt}}
%    \textbf{Git repository} & \textbf{:} & \href{\gitrepository}{\gitrepository} \\
...
%
%--- begin green table ---
\begin{longtable}{|>{\bfseries}p{6mm-12pt}|p{20mm-3mm-12pt}|p{\textwidth-20mm-3mm-12pt}|}
\caption{Table with green header} \\%
\rowcolor{mgreen1}
...
\begin{figure}[H]
    ...
    \caption{The menu bar of the \sugrid plugin}
\end{figure}
...
%--------------------------------------------------------------------------------
\subsubsection{Open GRID}
When selecting this option, you can choose \netcdf files which meet the SGRID (no padding) or UGRID standard.
Example files are the mesh- and map-files of the \dflowfm program (\file{$\ast$\_net.nc}, \file{$\ast$\_map.nc}).
The map-file contain time-series.
%--------------------------------------------------------------------------------
\subsubsection{Open HisCF}
When selecting this option, you can choose \netcdf files that meet the Climate and Forecast (CF) history file standard (Feature type "timeSeries").
...
After selecting \menu{Output\menuarrow Show map output} the window \window{Map Output Animation} will open, see as example \autoref{fig:map_output}.
\begin{figure}[H]
...
\begin{subfigure}[b]{0.48\textwidth}
	...
	\caption{Layout vector quantities.\label{fig:map_output_b}}
\end{subfigure}
    \caption{\window{Map Output Animation} window for scalars and vector.\label{fig:map_output}}
	% file used for this picture: FM-VZM_0004_map.nc.nc
\end{figure}

After pressing the \textbf{right} mouse button in this window, the window \window{Map Output Settings} will appear (see  \autoref{sec:map_output_settings} and \autoref{fig:map_output_settings}).

The colorramp can be changed by using the QGIS standard operations on colorramps.
%------------------------------------------------------------------------------
\subparagraph*{Add current view to QGIS layer panel}
\textit{This button is only enabled for scalar quantities.}
...
\begin{longtable}{p{35mm-12pt} p{\textwidth-35mm-12pt}}
	...
	Minimum value: \quad specify the minimum value for the scalar.\\	\textbf{Vector scaling} & \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{pictures/menu_help_about.png}
    \caption{About box}
\end{figure}
%--------------------------------------------------------------------------------
%\subsection{Trials}
...
\begin{figure}[H]
	...
	\caption{The \qgis layer panel after reading a \netcdf map-file.\label{fig:log_panel_layer}}
\end{figure}
%------------------------------------------------------------------------------
...
So the postprocessing program (\sugrid) need to compute the quantities of the vector field, like vector arrows and vector direction.

\textit{The "velocity magnitude" is given on the output file of \dflowfm and thus computed by \dflowfm.}
...
\url{https://github.com/mooiman/qgis_sgrid_ugrid}
%------------------------------------------------------------------------------
