...
//------------------------------------------------------------------------------
long GRID::open(QFileInfo filename, QProgressBar* pgBar)
{
    long ret_value = 1;
    m_fname = filename;  // filename without path, just the name
    m_grid_file_name = filename.absoluteFilePath();  // filename with complete path
...
#ifdef NATIVE_C
    int status = nc_open(m_grid_file_name, NC_NOWRITE, &this->m_ncid);
    if (status != NC_NOERR)
    {
    ...
    fprintf(stderr, "GRID::read()\n    Opened: %s\n", m_grid_file_name);
#else
    char* path_fname = strdup(m_fname.absoluteFilePath().toUtf8());
    int status = nc_open(path_fname, NC_NOWRITE, &m_ncid);
    if (status != NC_NOERR)
    {
        QMessageBox::critical(0, QString("Error"), QString("GRID::read()\n    Failed to open file: %1").arg(path_fname));
        return ret_value;
    }
    free(path_fname); path_fname = nullptr;
#endif
...
long GRID::read()
{
    ...
    else if (m_ftype == FILE_TYPE::SGRID ||
             m_ftype == FILE_TYPE::KISS)
    {
    ...
    {
        QMessageBox::warning(0, QString("Warning"), QString("GRID::read()\n    Supported file types are: UGRID and SGRID.\n    File: \"%1\" is n't").arg(m_grid_file_name));
        status = -1;
    }
    return status;
}
//------------------------------------------------------------------------------
long GRID::read_global_attributes()
{
    long status;
...
#ifdef NATIVE_C
    fprintf(stderr, "GRID::read_global_attributes()\n");
#endif    
...
//------------------------------------------------------------------------------
long GRID::get_global_attribute_value(std::string att_name, std::string* att_value)
{
...
//------------------------------------------------------------------------------
FILE_TYPE GRID::get_file_type()
{
    return m_ftype;
}
//------------------------------------------------------------------------------
#ifdef NATIVE_C
char* GRID::get_filename()
{
...
long GRID::read_ugrid_mesh()
{
    ...
    for (int i_var = 0; i_var < nvars; i_var++)
    {
        ...
        //QMessageBox::warning(0, QString("Warning"), QString("GRID::get_grid_mapping()\nProgress: (%1, %2) %3").arg(i_var + 1).arg(nvars).arg(var_name));
#endif
        length = 0;
...
//------------------------------------------------------------------------------
long GRID::read_kiss_mesh()
{
    long status = 1;
    return status;
}
//------------------------------------------------------------------------------
long GRID::set_grid_mapping_epsg(long epsg, std::string epsg_code)
{
    long status = 0;
    m_mapping->epsg = epsg;
...
//------------------------------------------------------------------------------
long GRID::get_count_times()
{
    size_t nr = time_series[0].nr_times;
    return (long)nr;
}
//------------------------------------------------------------------------------
std::vector<double> GRID::get_times()
{
    return time_series[0].times;
}
//------------------------------------------------------------------------------
QVector<QDateTime> GRID::get_qdt_times()  // qdt: Qt Date Time
{
    return qdt_times;
}//------------------------------------------------------------------------------
long GRID::read_ugrid_variables()
{
    // read the attributes of the variable  and the dimensions
#ifdef NATIVE_C
    fprintf(stderr, "GRID::read_variables()\n");
#endif    
    int ndims, nvars, natts, nunlimited;
    int status;
...
//------------------------------------------------------------------------------
long GRID::read_sgrid_variables()
{
    // read the attributes of the variable  and the dimensions
#ifdef NATIVE_C
    fprintf(stderr, "GRID::read_variables()\n");
...
//------------------------------------------------------------------------------
struct _mesh_variable * GRID::get_variables()
{
   return this->m_mesh_vars;
}
//------------------------------------------------------------------------------
struct _mesh_contact * GRID::get_mesh_contact()
{
    return this->m_mesh_contact;
}
//------------------------------------------------------------------------------
struct _ntw_nodes * GRID::get_connection_nodes()
{
    return this->m_ntw_nodes;
}
//------------------------------------------------------------------------------
struct _ntw_edges * GRID::get_network_edges()
{
    return this->m_ntw_edges;
}
//------------------------------------------------------------------------------
struct _ntw_geom * GRID::get_network_geometry()
{
    return this->m_ntw_geom;
}
//------------------------------------------------------------------------------
struct _mesh1d * GRID::get_mesh_1d()
{
#ifdef NATIVE_C
    fprintf(stderr, "GRID::get_mesh1d()\n");
#endif    
    return m_mesh1d;
}
//------------------------------------------------------------------------------
struct _mesh2d * GRID::get_mesh_2d()
{
#ifdef NATIVE_C
    fprintf(stderr, "GRID::get_mesh2d()\n");
#endif    
    return m_mesh2d;
}
//------------------------------------------------------------------------------
struct _mesh1d_string ** GRID::get_mesh1d_string()
{
#ifdef NATIVE_C
    fprintf(stderr, "GRID::get_mesh2d()\n");
#endif    
...
//------------------------------------------------------------------------------
struct _mesh2d_string ** GRID::get_mesh2d_string()
{
#ifdef NATIVE_C
    fprintf(stderr, "GRID::get_mesh2d()\n");
#endif    
...
//------------------------------------------------------------------------------
struct _mesh_contact_string ** GRID::get_mesh_contact_string()
{
#ifdef NATIVE_C
    fprintf(stderr, "GRID::get_mesh2d()\n");
#endif    
...
//------------------------------------------------------------------------------
struct _mapping * GRID::get_grid_mapping()
{
#ifdef NATIVE_C
    fprintf(stderr, "GRID::get_grid_mapping()\n");
#else
    //QMessageBox::warning(0, QString("Warning"), QString("GRID::get_grid_mapping()"));
#endif 
    return m_mapping;
...
//------------------------------------------------------------------------------
struct _variable * GRID::get_var_by_std_name(struct _mesh_variable * vars, std::string mesh, std::string standard_name)
{
...
//------------------------------------------------------------------------------
DataValuesProvider2D<double> GRID::get_variable_values(const std::string var_name)
{
...
//------------------------------------------------------------------------------
DataValuesProvider2D<double> GRID::get_variable_values(const std::string var_name, int array_index)
...
#ifdef NATIVE_C
    fprintf(stderr, "GRID::get_variable_values()\n");
#endif    
...
//------------------------------------------------------------------------------
DataValuesProvider3D<double> GRID::get_variable_3d_values(const std::string var_name)
...
//------------------------------------------------------------------------------
DataValuesProvider4D<double> GRID::get_variable_4d_values(const std::string var_name)
...
//==============================================================================
//
int GRID::read_mesh1d_attributes(struct _mesh1d_string* mesh1d_strings, int i_var, std::string var_name, int topology_dimension)
{
    int status = 1;
...
int GRID::read_mesh2d_attributes(struct _mesh2d_string* mesh2d_strings, int i_var, std::string var_name, int topology_dimension)
{
    int status = 1;
...
int GRID::determine_mesh1d_edge_length(struct _mesh1d* mesh1d, struct _ntw_edges* ntw_edges)
{
...
//------------------------------------------------------------------------------
std::vector<std::string> GRID::get_names(int ncid, std::string names, size_t count)
{
    int var_id;
    int status;
...
//------------------------------------------------------------------------------
std::vector<std::string> GRID::get_string_var(int ncid, std::string var_name)
{
    int varid;
    int ndims;
    int nr_names = 0;
...
//------------------------------------------------------------------------------
int GRID::read_variables_with_cf_role(int i_var, std::string var_name, std::string cf_role, int ndims, int* var_dimids)
{
    Q_UNUSED(var_dimids);
...
int GRID::read_geometry_attributes(struct _geom_string* geom_strings, int i_var, std::string var_name, int topology_dimension)
{
...
int GRID::read_composite_mesh_attributes(struct _mesh_contact_string* mesh_contact_strings, int i_var, std::string var_name)
{
    int status = -1;
...
int GRID::read_network_attributes(struct _ntw_string* ntw_strings, int i_var, std::string var_name, size_t topology_dimension)
{
    int status = 1;
...
int GRID::get_attribute_by_var_name(int ncid, std::string var_name, std::string att_name, std::string* att_value)
{
    int status = -1;
    int i_var;
...
//------------------------------------------------------------------------------
int GRID::get_attribute(int ncid, int i_var, char * att_name, char ** att_value)
{
    size_t length = 0;
    int status = -1;
...
//------------------------------------------------------------------------------
int GRID::get_attribute(int ncid, int i_var, char * att_name, std::string * att_value)
{
    size_t length = 0;
    int status = -1;
...
//------------------------------------------------------------------------------
int GRID::get_attribute(int ncid, int i_var, std::string att_name, std::string * att_value)
{
    size_t length = 0;
    int status = -1;
...
//------------------------------------------------------------------------------
int GRID::get_attribute(int ncid, int i_var, char * att_name, int * att_value)
{
    int status = -1;
...
//------------------------------------------------------------------------------
int GRID::get_attribute(int ncid, int i_var, char * att_name, long * att_value)
{
    int status = -1;
...
//------------------------------------------------------------------------------
int GRID::get_dimension(int ncid, char * dim_name, size_t * dim_length)
{
    int dimid;
    int status = -1;
...
//------------------------------------------------------------------------------
int GRID::get_dimension(int ncid, std::string dim_name, size_t * dim_length)
{
    int dimid;
    int status = -1;
...
//------------------------------------------------------------------------------
int GRID::get_dimension_var(int ncid, std::string var_name, size_t * dim_length)
{
...
std::vector<std::string>  GRID::get_dimension_names(int ncid, std::string var_name)
{
    int varid;
    int status = -1;
...
//------------------------------------------------------------------------------
std::vector<std::string> GRID::tokenize(const std::string& s, char c) {
    auto end = s.cend();
    auto start = end;
...
//
std::vector<std::string> GRID::tokenize(const std::string& s, std::size_t count)
{
...
char * GRID::strndup(const char *s, size_t n)
{
    char *result;
    size_t len = strlen(s);
...
double* GRID::permute_array(double* in_arr, std::vector<long> order, std::vector<long> dim)
{
    //
...
long GRID::get_index_in_c_array(long t, long l, long s, long xy, long time_dim, long layer_dim, long sed_dim, long xy_dim)
{
    Q_UNUSED(time_dim);
