...
//-----------------------------------------------------------------------------
//
sgrid_ugrid::sgrid_ugrid(QgisInterface* iface):
...
void sgrid_ugrid::onWillRemoveChildren(QgsLayerTreeNode* node, int indexFrom, int indexTo)
{
    Q_UNUSED(indexTo);
    ...
    if (children.size() == 0 && (name.contains("GRID - ") || name.contains("History -")))
    {
        if (indexFrom == 0)
        {
            ...
            QString timings_file(m_working_dir + QString("/sgrid_ugrid_timing.log"));
...
//-----------------------------------------------------------------------------
//
sgrid_ugrid::~sgrid_ugrid()
{
}
//
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::initGui()
{
#include "vsi.xpm"
    ...
    QDir program_env_dir = QDir(program_env);
    if (!program_env_dir.exists())
    {
    ...
    //------------------------------------------------------------------------------
    icon_open = get_icon_file(program_env, "/icons/file_open.png");
    ...
    icon_open_his_cf = get_icon_file(program_env, "/icons/file_open.png");
    ...
    icon_edit_1d_obs_points = get_icon_file(program_env, "/icons/edit_observation_points.png");
    ...
    connect(edit_action_1d_obs_points, &QAction::triggered, this, &sgrid_ugrid::edit_1d_obs_points);

    icon_experiment = get_icon_file(program_env, "/icons/experiment.png");
    ...
    connect(trial_experiment, &QAction::triggered, this, &sgrid_ugrid::experiment);

    icon_inspect = get_icon_file(program_env, "/icons/remoteolv_icon.png");
    ...
    icon_plotcfts = get_icon_file(program_env, "/icons/plotcfts.png");
    ...
    icon_mapoutput = get_icon_file(program_env, "/icons/map_output.png");
...
//-----------------------------------------------------------------------------
//
QIcon sgrid_ugrid::get_icon_file(QDir home_dir, QString file)
{
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::show_map_output(GRID* grid_file)
{
    //QMessageBox::information(0, "Information", QString("sgrid_ugrid::show_map_output()\nTime manager: %1.").arg(MapTimeManagerWindow::get_count()));
    ...
    {
        QMessageBox::warning(0, "Fatal error", QString("sgrid_ugrid::show_map_output().\nVariable grid_file is a nullptr."));
        return;
    }
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::edit_1d_obs_points()
{
}
//
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::experiment()
{
}
//
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::mapPropertyWindow()
{
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::ShowUserManual()
{
    QString program_files = QProcessEnvironment::systemEnvironment().value("ProgramFiles", "");
    QString company = getcompanystring_sgrid_ugrid();
    QString program_env = program_files + "/" + company + "/sgrid_ugrid";
    QDir program_env_dir = QDir(program_env);
    QString manual_path = program_env_dir.absolutePath();
    QString user_manual = manual_path + "/doc/" + QString("sgrid_ugrid_um.pdf");
...
Context:    -
-------------------------------------------------------*/
int sgrid_ugrid::QT_SpawnProcess(int waiting, char* prgm, char** args)
{
    int i = 0;
    int status;
    QStringList argList;
...
void sgrid_ugrid::openFile(QFileInfo ncfile)
{
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::open_file_his_cf()
{
    QString fname;
    QString* str = new QString();
    QFileDialog* fd = new QFileDialog();
...
void sgrid_ugrid::open_file_his_cf(QFileInfo ncfile)
{
    int ncid;
#if DO_TIMING == 1
    auto start = std::chrono::steady_clock::now();
#endif
    char* fname = strdup(ncfile.absoluteFilePath().toStdString().data());
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::open_file_mdu()
{
    QString* str = new QString();
    QFileDialog* fd = new QFileDialog();
    QStringList* list = new QStringList();
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::open_file_mdu(QString ffname)
{
    ...
    if (obs_file.size() != 0 && obs_file[0] != "null")
    {
        for (int i = 0; i < obs_file.size(); ++i)
        {
            ...
            else
            {
                ...
                std::string std_string = grid_file->get_filename().fileName().toStdString();
                if (std_string != ncfile[0])
                {
                    QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("Observations\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8((ncfile[0]).c_str())).arg(grid_file->get_filename().fileName()));
                    return;
                }
    ...
    if (extold_file_name.size() != 0 && extold_file_name[0] != "null" )
    {
        for (int i = 0; i < extold_file_name.size(); ++i)
        {
            ...
            else
            {
                ...
                if (std_string != ncfile[0])
                {
                    QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("External forcing (old format)\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8((ncfile[0]).c_str())).arg(grid_file->get_filename().fileName()));
                    return;
                }
                struct _mapping* mapping;
                mapping = grid_file->get_grid_mapping();
    ...
    if (ext_file_name.size() != 0 && ext_file_name[0] != "null")
    {
        for (int i = 0; i < ext_file_name.size(); ++i)
        {
            ...
            else
            {
                ...
                if (std_string != ncfile[0])
                {
                    QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("External forcing\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8((ncfile[0]).c_str())).arg(grid_file->get_filename().fileName()));
                    return;
                }
                struct _mapping* mapping;
                mapping = grid_file->get_grid_mapping();
    ...
    if (structure_file_name.size() != 0 && structure_file_name[0] != "null")
    {
        for (int i = 0; i < structure_file_name.size(); ++i)
        {
            ...
            else
            {
                ...
                if (std_string != ncfile[0])
                {
                    QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("Structures\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8((ncfile[0]).c_str())).arg(grid_file->get_filename().fileName()));
                    return;
                }
                struct _mapping* mapping;
                mapping = grid_file->get_grid_mapping();
    ...
    if (drypoints_file_name.size() != 0 && drypoints_file_name[0] != "null")
    {
        for (int i = 0; i < drypoints_file_name.size(); ++i)
        {
            ...
            else
            {
                ...
                if (std_string != ncfile[0])
                {
                    QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("Dry points\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8((ncfile[0]).c_str())).arg(grid_file->get_filename().fileName()));
                    return;
                }
                struct _mapping* mapping;
                mapping = grid_file->get_grid_mapping();
    ...
    if (prof_loc_file_name.size() != 0 && prof_loc_file_name[0] != "null")
    {
        for (int i = 0; i < prof_loc_file_name.size(); ++i)
        {
            ...
            else
            {
                ...
                if (std_string != ncfile[0])
                {
                    QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("Profile location\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8((ncfile[0]).c_str())).arg(grid_file->get_filename().fileName()));
                    return;
                }
                struct _mapping* mapping;
                mapping = grid_file->get_grid_mapping();
    ...
    if (cross_section_file_name.size() != 0 && cross_section_file_name[0] != "null")
    {
        for (int i = 0; i < cross_section_file_name.size(); ++i)
        {
            ...
            else
            {
                ...
                if (std_string != ncfile[0])
                {
                    QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("Observation cross-sections\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8((ncfile[0]).c_str())).arg(grid_file->get_filename().fileName()));
                    return;
                }
                struct _mapping* mapping;
                mapping = grid_file->get_grid_mapping();
    ...
    if (thin_dam_file_name.size() != 0 && thin_dam_file_name[0] != "null")
    {
        for (int i = 0; i < thin_dam_file_name.size(); ++i)
        {
            ...
            else
            {
                ...
                if (std_string != ncfile[0])
                {
                    QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("Thin dams\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8((ncfile[0]).c_str()).arg(grid_file->get_filename().fileName())));
                    return;
                }
                struct _mapping* mapping;
                mapping = grid_file->get_grid_mapping();
    ...
    if (fixed_weir_file_name.size() != 0 && fixed_weir_file_name[0] != "null")
    {
        for (int i = 0; i < fixed_weir_file_name.size(); ++i)
        {
            ...
            else
            {
                ...
                if (std_string != ncfile[0])
                {
                    QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("Fixed weirs\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8((ncfile[0]).c_str()).arg(grid_file->get_filename().fileName())));
                    return;
                }
                struct _mapping* mapping;
                mapping = grid_file->get_grid_mapping();
    ...
    if (cross_section_location_file_name.size() != 0 && cross_section_location_file_name[0] != "null")
    {
        for (int i = 0; i < cross_section_location_file_name.size(); ++i)
        {
            ...
            else
            {
                ...
                if (std_string != ncfile[0])
                {
                    QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("Cross-section\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8((ncfile[0]).c_str()).arg(grid_file->get_filename().fileName())));
                    return;
                }
                struct _mapping* mapping;
                mapping = grid_file->get_grid_mapping();
    ...
    if (retention_location_file_name.size() != 0 && retention_location_file_name[0] != "null")
    {
        for (int i = 0; i < retention_location_file_name.size(); ++i)
        {
            ...
            else
            {
                ...
                if (std_string != ncfile[0])
                {
                    //QMessageBox::warning(0, tr("sgrid_ugrid::open_file_mdu"), tr("Retention\nMesh files not the same:\n\"%1\",\n\"%2\".").arg(QString::fromUtf8(ncfile[0].c_str())).arg(grid_file->get_filename().fileName()));
                    return;
                }
                struct _mapping* mapping;
                mapping = grid_file->get_grid_mapping();
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::open_file_link1d2d_json()
{
    QString fname;
    QString* str = new QString();
    QFileDialog* fd = new QFileDialog();
    ...
    if (!canceled)
    {
        ...
        for (QStringList::Iterator it = QFilenames->begin(); it != QFilenames->end(); ++it) {
            fname = *it;
            open_file_link1d2d_json(QFileInfo(fname));
        }
...
void sgrid_ugrid::open_file_link1d2d_json(QFileInfo jsonfile)
{
    std::string fname = jsonfile.absoluteFilePath().toStdString();
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::open_file_obs_point_json()
{
    QString fname;
    QString* str = new QString();
    QFileDialog* fd = new QFileDialog();
    ...
    if (!canceled)
    {
        ...
        for (QStringList::Iterator it = QFilenames->begin(); it != QFilenames->end(); ++it) {
            fname = *it;
            open_file_obs_point_json(QFileInfo(fname));
        }
...
void sgrid_ugrid::open_file_obs_point_json(QFileInfo jsonfile)
{
    std::string fname = jsonfile.absoluteFilePath().toStdString();
    JSON_READER* pt_file = new JSON_READER(fname);
    if (pt_file == nullptr)
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::start_plotcfts()
{
    //QMessageBox::information(0, tr("sgrid_ugrid::start_plotcfts()"), tr("start_plotcfts."));

    QString company = getcompanystring_sgrid_ugrid();
    QString program_files = QProcessEnvironment::systemEnvironment().value("ProgramFiles", "");
    QString program_env = program_files + "/" + company + "/plotcfts";

    QString program = program_env + "/plotcfts.exe";
    QDir program_files_dir = QDir(program_env);
    ...
    QProcessEnvironment env;
    env.insert("PATH", program_env); // Add an environment variable
...
//-----------------------------------------------------------------------------
//
HISCF* sgrid_ugrid::get_active_his_cf_file(QString layer_id)
{
    ...
    if (layer_id != "")
    {
        //QMessageBox::information(0, "Information", QString("sgrid_ugrid::get_active_layer()\nActive layer: %1.").arg(active_layer->name()));
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::activate_layers()
{
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::activate_observation_layers()
{
...
//-----------------------------------------------------------------------------
//
char* sgrid_ugrid::stripSpaces(char* string)
{
...
//-----------------------------------------------------------------------------
//
void sgrid_ugrid::unload_vector_layers()
{
